...
/** ST7735_Message
 * The following function separates the display into two screens, a top and
 * and bottom half. There will be 8 lines in each half of the screen where
 * a string can be written.
 * @param device specifies top or bottom
 * @param line specifies the line number (0 to 7)
 * @param string is a null terminated pointer to string
 * @param value is a number to display
 */
void ST7735_Message(int32_t device, int32_t line, char *string, int32_t value){
	// Get semaphore (for lab 2)
	
	if(line > 7 || device < 0 || device > 1){
		return;
	}
	
	OS_bWait(&SemaDisplay);
	// If device = 0, that means top half. Device = 1 means bottom half.
	ST7735_SetCursor(0, device * 9 + line);		// Clear whole row
	ST7735_OutString("                    ");
	ST7735_SetCursor(0, device * 9 + line);		// The first parameter 0 means start string from left most column
	...
	if(value != 0){
	#endif
	char valueStr[11];		// null-term string buffer for value. The max number value can be is 2^31 - 1. This has 10 characters.
	int32_t idx = 0;
	...
	// Convert value to string starting at least significant digit
	// int 100 would be string "001" when loop is finished
	while(idx < 10 && value > 0){
		valueStr[idx++] = value % 10 + '0';
		value /= 10;		// Shift
	}
	
	// display valueStr in reverse order
	idx--;
	while(idx >= 0){
		ST7735_OutChar(valueStr[idx--]);
	}
	
	#ifndef DISPLAY_ZERO
	}
	#endif
	OS_bSignal(&SemaDisplay);
	// release semaphore (for lab 2)
}

/** ST7735_DivideScreen
 * Divides the display into top and bottom halves with a line
 */
...
//**************************************************************************************
// The following protoypes are functions that were provided in the original ST7735 files.
//**************************************************************************************
...
void ST7735_InitB(void) {
  ...
  ST7735_FillScreen(0);                 // set screen to black
	...
	OS_InitSemaphore(&SemaDisplay, 1);
}
...
// Multiple definitions error in target.c
/*
