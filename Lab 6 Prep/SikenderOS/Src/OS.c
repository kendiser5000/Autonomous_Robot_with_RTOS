...
#define JITTER1 1
uint32_t MaxJitter1, MaxJitter2;
#if JITTER1
// Jitter and Dump stuff for lab 3
#define JITTERSIZE 100
uint32_t JitterMax_Periodic1 = 0, JitterMax_Periodic2 = 0;
uint32_t JitterHistogram1[JITTERSIZE] = {0};
uint32_t JitterHistogram2[JITTERSIZE] = {0};
#define DUMPSIZE JITTERSIZE
uint32_t DataDump1 = 0, DataDump2 = 0;
uint32_t Dump1[DUMPSIZE] = {0};
uint32_t Dump2[DUMPSIZE] = {0};
...
//**LAB3******** PRIORITY SCHEDULING GLOBALS AND ARRAYS**************************************************************************
...
void Timer4A_Handler(void){
	...
	uint32_t jitter;
	if (Period1Counter < JITTERSIZE){
		int32_t difference = OS_TimeDifference(endTime, startTime);
		// check if jitter > period of task, in .1 us
		if (difference > Period1){
			jitter = (difference - Period1 +4 ) / 8;
		} else{
			jitter = (Period1 - difference + 4) / 8;
		}
		
		if (jitter > JitterMax_Periodic1)
			JitterMax_Periodic1 = jitter;
	...
	Period1Counter++;
	endTime = startTime;               // execute user task
...
void Timer1A_Handler(void){
	...
	uint32_t jitter;
	if (Period1Counter < JITTERSIZE){
		uint32_t difference = OS_TimeDifference(endTime, startTime);
		// check if jitter > period of task, in .1 us
		if (difference > Period2){
			jitter = (difference - Period2 + 4 ) / 8;
		} else{
			jitter = (Period2 - difference + 4) / 8;
		}
		
		if (jitter > JitterMax_Periodic2)
			JitterMax_Periodic2 = jitter;
		if (jitter >= JITTERSIZE){
			jitter = JITTERSIZE;
		}
...
*/
void Jitter(void) {
	MaxJitter1 = JitterMax_Periodic1;
	MaxJitter2 = JitterMax_Periodic2;
	//printf("Jitter 0.1us = %d\r\n", MaxJitter1);
	//printf("Jitter 0.1us = %d", MaxJitter2);
	ST7735_Message(0, 1, "Jitter2 0.1us = ", MaxJitter1);	// Jitter for Periodic Task 1
	ST7735_Message(0, 2, "Jitter1 0.1us = ", MaxJitter2);	// Jitter for Periodic Task 2
}
