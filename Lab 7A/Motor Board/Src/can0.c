...
// in this case there is just one type, but you could accept multiple ID types
  //CAN0_Setup_Message_Object(RCV_ID, MSG_OBJ_RX_INT_ENABLE, 4, NULL, RCV_ID, MSG_OBJ_TYPE_RX);
	#if RECEIVER
	CAN0_Setup_Message_Object((unsigned long) (LaserSensor1), MSG_OBJ_RX_INT_ENABLE, 4, NULL, (unsigned long) (LaserSensor1)	, MSG_OBJ_TYPE_RX);
	CAN0_Setup_Message_Object((unsigned long) (LaserSensor2), MSG_OBJ_RX_INT_ENABLE, 4, NULL, (unsigned long) (LaserSensor2)	, MSG_OBJ_TYPE_RX);
	CAN0_Setup_Message_Object((unsigned long) (LaserSensor3), MSG_OBJ_RX_INT_ENABLE, 4, NULL, (unsigned long) (LaserSensor3)	, MSG_OBJ_TYPE_RX);
	CAN0_Setup_Message_Object((unsigned long) (LaserSensor4), MSG_OBJ_RX_INT_ENABLE, 4, NULL, (unsigned long) (LaserSensor4)	, MSG_OBJ_TYPE_RX);
	CAN0_Setup_Message_Object((unsigned long) (Motors)		, MSG_OBJ_RX_INT_ENABLE, 4, NULL, (unsigned long) (Motors)	, MSG_OBJ_TYPE_RX);
	CAN0_Setup_Message_Object((unsigned long) (TEST)		, MSG_OBJ_RX_INT_ENABLE, 4, NULL, (unsigned long) (TEST)			, MSG_OBJ_TYPE_RX);
...
void CAN0_GetMail(CANID *receiveID, unsigned char data[4]){
	DataRCVFrame dataFrame;
	OS_Wait(&CANDataRdy);
	RCVFifo_Get(&dataFrame);
	*receiveID = dataFrame.RCVID;
	data[0] = dataFrame.RCVData[0];
	data[1] = dataFrame.RCVData[1];
	data[2] = dataFrame.RCVData[2];
	data[3] = dataFrame.RCVData[3];
}
